...
- **Unsourced detail is separated from sourced claims.** A topic now carries `FIELD_NOTES` alongside `CLAIMS`: the gdm3-vs-lightdm mapping has no recorded citation, so it is rendered under its own "unsourced — not claims" heading rather than riding along inside a claim that cites only the generic `systemctl` manual. A test pins the separation.
...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`jetson vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/jetson/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `jetson/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=jetson` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/jetson/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/jetson/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: jetson`,
