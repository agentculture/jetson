...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope jetson
...
scope:** the default personal scope is **private** (`--scope jetson
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `jetson`). Private records are served only to a recall in the same
