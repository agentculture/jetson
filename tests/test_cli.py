"""Smoke tests for the jetson CLI entry point and its verbs."""
...
from jetson.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: jetson" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "jetson whoami" in payload["markdown"]
