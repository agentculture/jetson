...
jetson vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. jetson
...
#   - consumer-identifying prose: `guildmaster` → `jetson` (NOT
...
If a re-sync would lose a jetson adaptation, lift the change
...
#  `which jetson vendors from guildmaster`).
