...
boots into is the **systemd default target**:
...
## Change it persistently (takes effect next boot)
...
- Which display manager ships per JetPack release (`gdm3` vs `lightdm`, and from
  which L4T version) is not verified release-by-release — hence the `medium`
  confidence on `display-manager-may-be-disabled`.
...
claims (each with commands, source ids, and a confidence), a source table, and an
explicit gap list. That shape is the smallest thing that lets a first topic carry
